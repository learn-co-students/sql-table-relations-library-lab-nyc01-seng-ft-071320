...
def select_series_title_with_most_human_characters
  ...
  JOIN characters ON characters.id = character_books.character_id GROUP BY characters.species ORDER BY COUNT(characters.species = 'human') DESC LIMIT 1;"
end

def select_character_names_and_number_of_books_they_are_in
  "SELECT characters.name, COUNT(*) AS book_count FROM characters JOIN character_books ON characters.id = character_books.character_id
   GROUP BY character_books.character_id ORDER BY book_count DESC;"
end
